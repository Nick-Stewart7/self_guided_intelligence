    ...
    async def mind_loop(self):
        ...
        while self.running:
            try:
                ...
                # Check if mind loop should be enabled
                if not config.aria.mind_loop_enabled:
                    print("Mind loop disabled in configuration. Stopping.")
                    break
...
async def health_check():
    ...
    try:
        ...
        if not aws_accessible:
            health_status["checks"]["aws_error"] = aws_error
